"""Kinect v1 (Xbox 360) через libfreenect — базовый датчик объекта. Заготовка.
...
высоте примерно 3 мм. Запасной вариант — Kinect v2 (kinect2.py): консоль одна
на оба датчика, меняется только высота подвеса.
...
Реализовать на этапе 1: открытие устройства, поток глубины 30 к/с в мм,
цветной кадр 640×480 по желанию, перезапуск при потере кадров, наклон
мотора в 0°, подсчёт к/с и ошибок для health().
"""
