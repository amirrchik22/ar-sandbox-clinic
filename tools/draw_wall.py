...
Параметры — в блоке P ниже. Кинект v1: датчик 1,20 м над песком, Г-образная консоль.
"""
import math, pathlib

P = dict(
    GAP=0.020, PLY=0.018, D=0.750, L=1.000, WALL_H=0.250, SAND=0.150,
    SENS_ABOVE=1.20, FOV=(57, 43), SENS_RES=640, SENS_NAME="Kinect v1",
...
def geometry(p):
    ...
    g["SENS_Z"] = p["SAND"] + p["SENS_ABOVE"]; g["ARM_Z"] = g["SENS_Z"] + 0.05      # верх датчика + консоль
    ...
    g["wall_in_frame_z"] = g["SENS_Z"] - g["XC"] / math.tan(math.radians(p["FOV"][1] / 2))
    dy = g["LENS_Z"] - g["ARM_Z"]
...
def side_view(p, g):
    ...
    sc = (X(g["XC"]), Y(g["SENS_Z"]))
    a(f'<polygon points="{sc[0]:.1f},{sc[1]:.1f} {X0},{Y(g["wall_in_frame_z"]):.1f} {X0},{Y(p["SAND"]):.1f} {X(g["XC"]+g["hx"]):.1f},{Y(p["SAND"]):.1f}" fill="#1a8c9c" opacity="0.08"/>')
    a(f'<line x1="{sc[0]:.1f}" y1="{sc[1]:.1f}" x2="{X0}" y2="{Y(g["wall_in_frame_z"]):.1f}" stroke="#1a8c9c" stroke-width="1.3" stroke-dasharray="5 3"/>')
    a(f'<line x1="{sc[0]:.1f}" y1="{sc[1]:.1f}" x2="{X(g["XC"]+g["hx"]):.1f}" y2="{Y(p["SAND"]):.1f}" stroke="#1a8c9c" stroke-width="1.3" stroke-dasharray="5 3"/>')
    ...
    a(f'<rect x="{X0}" y="{Y(2.30):.1f}" width="{0.04*S:.1f}" height="{(2.30-(arm_top-0.10))*S:.1f}" fill="#6f7a86"/>')     # рейка
    a(f'<rect x="{X0}" y="{Y(arm_top):.1f}" width="{(g["XC"]+0.04)*S:.1f}" height="{0.04*S:.1f}" fill="#6f7a86"/>')         # консоль
    a(f'<rect x="{X(g["XC"]-0.02):.1f}" y="{Y(arm_top):.1f}" width="{0.04*S:.1f}" height="{0.04*S:.1f}" fill="#4f5a66"/>')     # поперечина (торец)
    a(f'<rect x="{X(g["XC"]-0.033):.1f}" y="{Y(g["SENS_Z"]+0.033):.1f}" width="{0.066*S:.1f}" height="{0.067*S:.1f}" fill="#1a8c9c"/>')
    ...
    def vdim(x, z1, z2, label):
        ...
        a(f'<text transform="translate({x-5} {Y((z1+z2)/2):.1f}) rotate(-90)" text-anchor="middle" fill="#5d6771" font-size="10.5">{label}</text>')
    def hline(y, x1, x2, label, lx):
        ...
        a(f'<text x="{lx:.1f}" y="{y+4:.1f}" fill="#5d6771" font-size="10.5">{label}</text>')
    a(f'<line x1="{X(g["XC"]+g["hx"]):.1f}" y1="{Y(p["SAND"]):.1f}" x2="612" y2="{Y(p["SAND"]):.1f}" stroke="#5d6771" stroke-dasharray="2 3" opacity="0.7"/>')
    a(f'<line x1="{X(g["XC"]+0.033):.1f}" y1="{Y(g["SENS_Z"]):.1f}" x2="612" y2="{Y(g["SENS_Z"]):.1f}" stroke="#5d6771" stroke-dasharray="2 3" opacity="0.7"/>')
    ...
    for xm in (0.0, p["X_LENS"], g["XC"], g["far"], p["GAP"] + g["OUT_D"]):
    ...
    hline(Y0 + 48, 0, g["XC"], f"центр ящика и ось датчика {g['XC']*1000:.0f} мм (ящик 1000×600: {(p['GAP']+(0.6+2*p['PLY'])/2)*1000:.0f} мм)", X(g["XC"]) + 8)
    hline(Y0 + 66, 0, p["X_LENS"], f"объектив {p['X_LENS']*1000:.0f} мм от стены; ящик снаружи {p['GAP']*1000:.0f}…{(p['GAP']+g['OUT_D'])*1000:.0f} мм, зазор под плинтус {p['GAP']*1000:.0f} мм", X(p["X_LENS"]) + 8)
    ...
    lbl = lambda x, y, t: a(f'<text x="{x:.1f}" y="{y:.1f}" fill="#1d232a" font-size="11">{t}</text>')
    a(f'<line x1="{X0}" y1="10" x2="{X(g["XC"]):.1f}" y2="10" stroke="#5d6771"/><line x1="{X0}" y1="6" x2="{X0}" y2="14" stroke="#5d6771"/><line x1="{X(g["XC"]):.1f}" y1="6" x2="{X(g["XC"]):.1f}" y2="14" stroke="#5d6771"/>')
    lbl(X(g["XC"]) + 8, 14, f"вынос консоли до оси датчика {g['XC']*1000:.0f} мм; консоль Г-образная: от стены на Y = +{p['ARM_Y']*1000:.0f}, поперечина к центру")
    lbl(X0 + 48, Y(2.24) + 2, "рейка 40×40, три анкера M8, верх 2,30 м; проектор на своей пластине по центру")
    ...
    lbl(X(g["XC"]) + 45, Y(g["SENS_Z"]), f"датчик {p['SENS_NAME']} над центром ящика, {p['SENS_ABOVE']:.2f} м над песком")
    lbl(X(g["XC"]) + 45, Y(g["SENS_Z"] - 0.05), f"видит {g['cov_l']:.2f}×{g['cov_d']:.2f} м ({g['px_mm']:.1f} мм на пиксель), стена в кадре ниже {g['wall_in_frame_z']:.2f} м")
    lbl(X(p["GAP"] + g["OUT_D"]) + 8, Y(0.82), "ребёнок 5 лет у переднего борта:")
    lbl(X(p["GAP"] + g["OUT_D"]) + 8, Y(0.77), "голова вне луча проектора, тени только от рук")
...
def top_view(p, g):
    ...
    OUT_D, OUT_L, XC, ARM_Y = g["OUT_D"], g["OUT_L"], g["XC"], p["ARM_Y"]
    o = []; a = o.append
    ...
    a(f'<text x="{px(-OUT_L/2-0.6)+4:.1f}" y="{py(p["GAP"]+OUT_D+0.6)-6:.1f}" fill="#5d6771" font-size="10.5">свободная зона 0,6 м с трёх сторон</text>')
    ...
    x0 = max(0.0, XC - g["hx"])
    a(f'<rect x="{px(-g["hz"]):.1f}" y="{py(x0):.1f}" width="{2*g["hz"]*S:.1f}" height="{(XC+g["hx"]-x0)*S:.1f}" fill="#1a8c9c" opacity="0.06" stroke="#1a8c9c" stroke-width="1.3" stroke-dasharray="5 3"/>')
    ...
    a(f'<text x="{px(0):.1f}" y="{py(0.66)+4:.1f}" text-anchor="middle" fill="#5d6771">песок 1000 × {p["D"]*1000:.0f} внутри, борт 18 мм</text>')
    ...
    a(f'<text x="{px(0):.1f}" y="{py(XC+0.15)+4:.1f}" text-anchor="middle" fill="#8a5a12" font-size="10">оранжевое у стены — сечение луча на высоте консоли {g["ARM_Z"]:.2f} м</text>')
    a(f'<rect x="{px(ARM_Y-0.02):.1f}" y="{py(0):.1f}" width="{0.04*S:.1f}" height="{(XC+0.04)*S:.1f}" fill="#6f7a86"/>')                  # консоль вдоль X
    a(f'<rect x="{px(-0.06):.1f}" y="{py(XC-0.02):.1f}" width="{(ARM_Y+0.08)*S:.1f}" height="{0.04*S:.1f}" fill="#4f5a66"/>')            # поперечина к центру
    a(f'<rect x="{px(-0.1245):.1f}" y="{py(XC-0.033):.1f}" width="{0.249*S:.1f}" height="{0.066*S:.1f}" fill="#1a8c9c"/>')             # датчик по центру
    ...
    def hdim(x, y1, y2, label):
        ...
        a(f'<text x="{px((y1+y2)/2):.1f}" y="{Y+13:.1f}" text-anchor="middle" fill="#5d6771" font-size="10.5">{label}</text>')
    def vdim(y, x1, x2, label):
        ...
        a(f'<text transform="translate({Xp-5:.1f} {py((x1+x2)/2):.1f}) rotate(-90)" text-anchor="middle" fill="#5d6771" font-size="10.5">{label}</text>')
    ...
    hdim(1.57, 0, ARM_Y, f"консоль +{ARM_Y*1000:.0f} мм от центра, поперечина к центру")
    vdim(-0.72, 0, XC, f"ось датчика {XC*1000:.0f}")
    ...
    lbl = lambda x, y, t, anchor="start", size=11, color="#1d232a": a(f'<text x="{x:.1f}" y="{y:.1f}" text-anchor="{anchor}" fill="{color}" font-size="{size}">{t}</text>')
    lbl(px(-0.17), py(0.09) + 4, "проектор, Y = 0", "end")
    lbl(px(ARM_Y + 0.04), py(0.12) + 4, f"рейка, Y = +{ARM_Y*1000:.0f}", "start", 10.5)
    lbl(px(-0.16), py(XC) + 4, f"датчик {p['SENS_NAME']}, X = {XC*1000:.0f}", "end")
    lbl(px(0.35), py(0.03) + 3, "кабель-канал → тумба", "start", 9.5, "#5d6771")
    ...
    notes = [f"Проектор на настенной пластине по центру ящика (Y = 0), объектив на {p['X_LENS']*1000:.0f} мм от стены и {g['LENS_Z']:.2f} м от пола.",
             f"Консоль Г-образная: от стены на Y = +{ARM_Y*1000:.0f}, поперечина к центру на X = {XC*1000:.0f}. На высоте консоли луч занимает",
             f"±{g['beam_y_arm']*1000:.0f} мм по Y и до {g['beam_x_arm']*1000:.0f} мм от стены — консоль, поперечина и датчик вне луча, тени нет.",
             f"Датчик {p['SENS_NAME']} над центром ящика на {g['SENS_Z']:.2f} м от пола; поле {g['cov_l']:.2f} × {g['cov_d']:.2f} м, {g['px_mm']:.1f} мм на пиксель.",
...
if __name__ == "__main__":
    ...
    keys = ["XC", "SENS_Z", "ARM_Z", "LENS_Z", "tilt", "near", "far", "cov_l", "cov_d", "px_mm", "wall_in_frame_z", "beam_x_arm", "beam_y_arm"]
